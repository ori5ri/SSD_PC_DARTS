#### DARTS(normal, reduction cell) base SSD300 model ####
...
class Cell_ceil(nn.Module):
    ...
    def _compile(self, C, op_names, indices, concat, reduction):
        ...
        for name, index in zip(op_names, indices):
            stride = 2 if reduction and index < 2 else 1
            op = Ceil_OPS[name](C, stride, True)
            self._ops += [op]
        self._indices = indices
...
class DARTSBase(nn.Module):
    ...
    def __init__(self):
        ...
        self.cell1 = Cell(genotype, C_prev_prev=16, C_prev=16, C=16, reduction=False, reduction_prev=False) # torch.Size([1, 64, 300, 300])
        self.cell2 = Cell(genotype, C_prev_prev=16, C_prev=64, C=16, reduction=False, reduction_prev=False) # torch.Size([1, 64, 300, 300])
        self.cell3 = Cell(genotype, C_prev_prev=64, C_prev=64, C=32, reduction=True, reduction_prev=False)  # torch.Size([1, 128, 150, 150]) # reduction cell

        self.cell4 = Cell(genotype, C_prev_prev=64, C_prev=128, C=32, reduction=False, reduction_prev=True) # torch.Size([1, 128, 150, 150])
        self.cell5 = Cell(genotype, C_prev_prev=128, C_prev=128, C=32, reduction=False, reduction_prev=False) # torch.Size([1, 128, 150, 150])
        self.cell6 = Cell(genotype, C_prev_prev=128, C_prev=128, C=64, reduction=True, reduction_prev=False) # torch.Size([1, 256, 75, 75]) # reduction cell

        self.cell7 = Cell(genotype, C_prev_prev=128, C_prev=256, C=64, reduction=False, reduction_prev=True) # torch.Size([1, 256, 75, 75])
        self.cell8 = Cell(genotype, C_prev_prev=256, C_prev=256, C=64, reduction=False, reduction_prev=False) # torch.Size([1, 256, 75, 75])
        self.cell9 = Cell_ceil(genotype, C_prev_prev=256, C_prev=256, C=128, reduction=True, reduction_prev=False) # torch.Size([1, 512, 38, 38]) # reduction cell - ceil mode

        self.cell10 = Cell(genotype, C_prev_prev=256, C_prev=512, C=128, reduction=False, reduction_prev=True) # torch.Size([1, 512, 38, 38])
        self.cell11 = Cell(genotype, C_prev_prev=512, C_prev=512, C=128, reduction=False, reduction_prev=False) # torch.Size([1, 512, 38, 38])
        self.cell12 = Cell(genotype, C_prev_prev=512, C_prev=512, C=128, reduction=True, reduction_prev=False)  # torch.Size([1, 512, 19, 19]) # reduction cell

        self.cell13 = Cell(genotype, C_prev_prev=512, C_prev=512, C=128, reduction=False, reduction_prev=True) # torch.Size([1, 512, 19, 19])
        self.cell14 = Cell(genotype, C_prev_prev=512, C_prev=512, C=128, reduction=False, reduction_prev=False) # torch.Size([1, 512, 19, 19])
    ...
    def forward(self, image):
        s0 = s1 = self.stem(image)  # torch.Size([1, 16, 300, 300])
        #print("stem : ", s0.size())
        #print("stem : ", s1.size())
        s0, s1 = s1, self.cell1(s0, s1)  # torch.Size([1, 64, 300, 300])
        #print("cell1 : ", s1.size())
        s0, s1 = s1, self.cell2(s0, s1)  # torch.Size([1, 64, 300, 300])
        #print("cell2 : ", s1.size())
        s0, s1 = s1, self.cell3(s0, s1)  # torch.Size([1, 128, 150, 150])
        #print("cell3 : ", s1.size())

        s0, s1 = s1, self.cell4(s0, s1)  # torch.Size([1, 128, 150, 150])
        #print("cell4 : ", s1.size())
        s0, s1 = s1, self.cell5(s0, s1)  # torch.Size([1, 128, 150, 150])
        #print("cell5 : ", s1.size())
        s0, s1 = s1, self.cell6(s0, s1)  # torch.Size([1, 256, 75, 75])
        #print("cell6 : ", s1.size())

        s0, s1 = s1, self.cell7(s0, s1)  # torch.Size([1, 256, 75, 75])
        #print("cell7 : ", s1.size())
        s0, s1 = s1, self.cell8(s0, s1)  # torch.Size([1, 256, 75, 75])
        #print("cell8 : ", s1.size())
        # ceil mode # torch.Size([1, 512, 38, 38])
        s0, s1 = s1, self.cell9(s0, s1)
        #print("cell9 : ", s1.size())

        s0, s1 = s1, self.cell10(s0, s1)  # torch.Size([1, 512, 38, 38])
        #print("cell10 : ", s1.size())
        s0, s1 = s1, self.cell11(s0, s1)  # torch.Size([1, 512, 38, 38])
        #print("cell11 : ", s1.size())
        conv4_3_feats = s1  # torch.Size([1, 512, 38, 38])
        #print("conv4_3_feats : ", conv4_3_feats.size())
        s0, s1 = s1, self.cell12(s0, s1)  # torch.Size([1, 512, 19, 19])
        #print("cell12 : ", s1.size())

        s0, s1 = s1, self.cell13(s0, s1)  # torch.Size([1, 512, 19, 19])
        #print("cell13 : ", s1.size())
        s0, s1 = s1, self.cell14(s0, s1)  # torch.Size([1, 512, 19, 19])
        #print("cell14 : ", s1.size())
        out = self.pool5(s1)  # torch.Size([1, 512, 19, 19])
        #print("pool5 : ", out.size())

        out = F.relu(self.conv6(out))  # torch.Size([1, 1024, 19, 19])
        #print("conv6 : ", out.size())
        conv7_feats = F.relu(self.conv7(out))  # torch.Size([1, 1024, 19, 19])
        #print("conv7 : ", conv7_feats.size())
